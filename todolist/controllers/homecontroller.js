const TodoLists = require('../models/task');

// function for redirecting to main home page

module.exports.home = function(req,res){
    //fetching using mongoose
    TodoLists.find({},function(err,todo){
        if(err){
            console.log('Error in fetching data');
        }
        return res.render('homePage',{
            title:"Home",
            todolist:todo
        })
    })
}

//function for new date
function dateValue(dueDate){
    let months=['jan','feb','mar','Apr','May','june','july','aug','sept','oct','nov','dec'];
    // static value for implementing month value
    newdate = '';
    let monapp = '';
    //Checking months
    if(dueDate[1] == '01'){
        monapp=months[0];
    }
    else if(dueDate[1] == '02'){
        monapp=months[1];
    }else if(dueDate[1] == '03'){
        monapp=months[2];
    }else if(dueDate[1] == '04'){
        monapp=months[3];
    }else if(dueDate[1] == '04'){
        monapp=months[3];
    }else if(dueDate[1] == '05'){
        monapp=months[4];
    }else if(dueDate[1] == '06'){
        monapp=months[5];
    }else if(dueDate[1] == '07'){
        monapp=months[6];
    }else if(dueDate[1] == '08'){
        monapp=months[7];
    }else if(dueDate[1] == '09'){
        monapp=months[8];
    }else if(dueDate[1] == '10'){
        monapp=months[9];
    }else if(dueDate[1] == '11'){
        monapp=months[10];
    }else if(dueDate[1] == '12'){
        monapp=months[11];
    }
    newdate= dueDate[2]+'-'+monapp+'-'+dueDate[0]; // display date in dd-mm-yy formate
    return newdate;


}

//function for creating todo list

module.exports.createTodo = function(req,res){
    dueDate = req.body.dateValue.split('-'); //splitting date and taking month value
    // console.log(`dueDate : ${dueDate}`);
    let newdate = '';
    newdate=dateValue(dueDate);
    // console.log(`newDate: ${newdate}`);
    TodoLists.create({//creating new todo and storing on db
        desc:req.body.desc,
        category:req.body.category,
        dueDate:newdate
    },function(err,newArr){
      if(err){
        console.log('opps error occured');
      }
      return res.redirect('/');
    })

}

// function for deleting todo list
module.exports.deleteTodo = function(req,res){ 
    sp = req.query.id; // getting the id from ui
    newsp = sp.split(','); 
    for(let i=0;i<newsp.length;i++){ // looping over newsp  to delete all the checked value
        TodoLists.findByIdAndDelete(newsp[i],function(err){
            if(err){
                console.log('err')
                return;
            }
        })
    }
return res.redirect('/');
}

// fetching data for edit page
module.exports.editPage = function(req,res){
    console.log(`aaa:${req.query}`);
    TodoLists.findById(req.query.id,function(err,todolist){
        if(err){
            console.log('hi man!! it an error');
            return;
        }
        return res.render('editPage',{
            title:'Edit Page',
            todolist:todolist
        })

    })
}

// function for updating data in db
module.exports.editDetails = function(req,res){
    dueDate =  req.body.dueDate.split('-');
    let newdate='';
    newdate=dateValue(dueDate);
    TodoLists.updateOne({_id:req.query.id},{$set:{
        desc:req.body.desc,
        category:req.body.category,
        dueDate:newdate
    }},function(err,todoData){
        if(err){
            console.log('Error while updating data');
            return;
        }
        return res.redirect('/');
    });

}